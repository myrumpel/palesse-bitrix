...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] <> '' && (!isset($_POST["PARAMS_HASH"]) || $arResult["PARAMS_HASH"] === $_POST["PARAMS_HASH"])) {
	...
	if (check_bitrix_sessid()) {
		if (empty($arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $arParams["REQUIRED_FIELDS"])) {
			...
			if ((empty($arParams["REQUIRED_FIELDS"]) || in_array("EMAIL", $arParams["REQUIRED_FIELDS"])) && mb_strlen($_POST["Phone"]) <= 1)
				$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_EMAIL");
			if ((empty($arParams["REQUIRED_FIELDS"]) || in_array("Topic", $arParams["REQUIRED_FIELDS"])) && mb_strlen($_POST["Topic"]) <= 3)
				$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_MESSAGE");
		}
		if (mb_strlen($_POST["Phone"]) > 1 && !check_email($_POST["Phone"]))
		...
		$arResult["AUTHOR_EMAIL"] = htmlspecialcharsbx($_POST["Phone"]);
	} else
