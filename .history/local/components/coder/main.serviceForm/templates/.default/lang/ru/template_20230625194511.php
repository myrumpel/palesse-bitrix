<?
$MESS['MFT_NAME'] = "Ваше имя";
$MESS['MFT_EMAIL'] = "Ваш E-mail";
$MESS['MFT_MESSAGE'] = "Сообщение";
$MESS['MFT_CAPTCHA'] = "Защита от автоматических сообщений";
$MESS['MFT_CAPTCHA_CODE'] = "Введите слово на картинке";
$MESS['MFT_SUBMIT'] = "Отправить";
$MESS['MF_OK_MESSAGE'] = "Обращение отправлено";
$MESS['MF_REQ_NAME'] = "Укажите имя";
$MESS['MF_REQ_EMAIL'] = "Укажите E-mail, на который хотите получить ответ.";
$MESS['MF_REQ_MESSAGE'] = "Укажите тему обращения (от 8 символов)";
$MESS['MF_REQ_PHONE'] = "Укажите номер телефона";
$MESS['MF_EMAIL_NOT_VALID'] = "Указанный E-mail некорректен.";
$MESS['MF_CAPTCHA_WRONG'] = "Неверно указан код защиты от автоматических сообщений.";
$MESS['MF_CAPTHCA_EMPTY'] = "Не указан код защиты от автоматических сообщений.";
$MESS['MF_SESS_EXP'] = "Ваша сессия истекла. Отправьте сообщение повторно.";

$MESS['MF_REQ_SERVICE_NAME'] = "Укажите имя";
$MESS['MF_REQ_SERVICE_PHONE'] = "Укажите номер телефона";

$MESS['MF_REQ_SERVICE_JOB'] = "Укажите требуемые работы";
$MESS['MF_REQ_SERVICE_COMPANY'] = "Укажите хозяйство";
?>