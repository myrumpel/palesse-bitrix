...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] <> '' && (!isset($_POST["PARAMS_HASH"]) || $arResult["PARAMS_HASH"] === $_POST["PARAMS_HASH"])) {
	...
	if (check_bitrix_sessid()) {
		if (empty($arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $arParams["REQUIRED_FIELDS"])) {
			...
			if ((empty($arParams["REQUIRED_FIELDS"]) || in_array("PHONE", $arParams["REQUIRED_FIELDS"])) && mb_strlen($_POST["Phone"]) <= 5)
				$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_PHONE");
			if ((empty($arParams["REQUIRED_FIELDS"]) || in_array("TOPIC", $arParams["REQUIRED_FIELDS"])) && mb_strlen($_POST["Topic"]) <= 8)
		...
		$arResult["AUTHOR_PHONE"] = htmlspecialcharsbx($_POST["user_email"]);
	} else
