	...
	<div class="container-fluid">
		<div class="feedback__bg"
			...
			<div class="container">
					...
					<div class="feedback__content">
						<form action="<?= POST_FORM_ACTION_URI ?>" method="POST" class="feedback__form" id='feedback-form'>
							...
							<label for='feedback-name' class='feedback-label form-item'>
								...
								<input class="feedback__input feedback__name input-field-index" maxlength='40' id='feedback-name' <? if (!empty($arResult["ERROR_MESSAGE"][0])) { ?> style="border: 1px solid red" <? } else { ?>
							...
							<label for='feedback-ph one' class='feedback-label form-item'>
								<input class="feedback__input feedback__phone input-field-index" id='feedback-phone' type="tel" <? if (!empty($arResult["ERROR_MESSAGE"][1])) { ?> style="border: 1px solid red" <? } else { ?>
										style="border: 1px solid #4a4f59" <? } ?> name='Phone' placeholder='Телефон' maxlength='20'>
							</label>
							<label for='feedback-topic' class='feedback-label form-item'>
								<input class="feedback__input feedback__name input-field-index" maxlength='80' id='feedback-topic' <? if (!empty($arResult["ERROR_MESSAGE"][2])) { ?> style="border: 1px solid red" <? } else { ?>
				...
				<!-- <?
				echo '<pre>';
				print_r($arResult["ERROR_MESSAGE"][0]);
				echo '</pre>';
				?> -->
			</div>
		</div>
