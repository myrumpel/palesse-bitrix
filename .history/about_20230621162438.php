<?php
global $APPLICATION;
require($_SERVER['DOCUMENT_ROOT'].'/bitrix/header.php');
$APPLICATION->SetTitle('О компании');
?>



					<section class="about">
						<div class="about__inner">
							<div class="container">
								<div class="about__headline">
									<h1 class="about__title">Торговый дом ПАЛЕССЕ-ЮГ</h1>
									<h2 class="about__subtitle">Профессионализм и ответственность коллектива позволили Торговому Дому «Палессе-юг»
										получать стабильный положительный результат на протяжении 9 лет работы, хорошо зарекомендовать себя
										как надёжный поставщик и предприятие ремонтно-сервисного направления у аграриев Российской Федерации.</h2>
								</div>
							</div>
							<section class="about-leaders">

								<div class="container-fluid">
									<div class="about-leaders__bg" style='background-image: url(<?=SITE_TEMPLATE_PATH;?>/assets/img/pages-bg-pictures/about-main.jpg);'>
										<div class="container">
											<div class="about-leaders__inner">
												<div class="about-leaders__content">
													<h4 class="about-leaders__content-title"> МЫ — ЛИДЕРЫ</h4>
													<div class="about-leaders__content-text">
														<p class=''>За годы работы нас выбрали</p>
														<p class=''><span>711</span>
															 сельхозтоваропроизводителей
															при обновлении комбайнового парка,</p>
														<p class=''><span>685</span>  сельхозтоваропроизводителей при приобретении
															тракторов, более</p>
														<p class=''><span>815</span>
															 сельхозпроизводителей, приобретавших прочую технику.</p>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
								<div class="container">
									<div class="mob-container"></div>
								</div>
							</section>
							<section class="what-do">
								<div class="container">
									<h2 class="what-do__title">
										Что мы делаем?
									</h2>
									<ul class="what-do__list">
										<li class="what-do__item">
											<a class="what-do__link" href="/ways.php">
												<div class="what-do__item-inner">
													<div class="what-do__item-title">Продаём</div>
													<div class="what-do__item-content">Мы реализуем сельскохозяйственную технику: зерно — и кормоуборочных
														комбайнов, тракторов различных марок, почвообрабатывающих орудий, а также техники предназначенной
														для прямого посева и возделывания культур.</div>
													<div class="what-do__item-arrow">
														<svg class='about-what-do-arrow-keeper about-what-do-arrow-absolute'>
															<use class='about-what-do-arrow' xlink:href='<?=SITE_TEMPLATE_PATH;?>/assets/img/icons/icons.svg#svg-arrow-right'></use>
														</svg>
													</div>
												</div>
											</a>
										</li>
										<li class="what-do__item">
											<a class="what-do__link" href="/service.php">
												<div class="what-do__item-inner">
													<div class="what-do__item-title">Гарантируем</div>
													<div class="what-do__item-content">
														<ol class="what-do__item-content-list">
															<li class='what-do__item-content-item'>Досборку новой техники «под ключ», её обкатка в строгом
																соответствии с регламентом, утверждённым заводом-изготовителем, как на базе сервисный центр,
																так и на территории потребителя;</li>
															<li class='what-do__item-content-item'>Бесплатное проведение инструктажа механизаторов,
																консультативная помощь в технических и эксплуатационных вопросах.</li>
															<li class='what-do__item-content-item'> Доставка запасных частей «в поле» в период уборки.
															</li>
															<li class='what-do__item-content-item'> Проведение регламентного технического обслуживания;
															</li>
															<li class='what-do__item-content-item'> Постгарантийное обслуживание техники.
															</li>
														</ol>



													</div>
													<div class="what-do__item-arrow">
														<svg class='about-what-do-arrow-keeper about-what-do-arrow-absolute'>
															<use class='about-what-do-arrow' xlink:href='<?=SITE_TEMPLATE_PATH;?>/assets/img/icons/icons.svg#svg-arrow-right'></use>
														</svg>
													</div>
												</div>
											</a>
										</li>
										<li class="what-do__item">
											<a class="what-do__link" href="/spare-parts.php">
												<div class="what-do__item-inner">
													<div class="what-do__item-title">Обеспечиваем</div>
													<div class="what-do__item-content">На нашем складе представлен широкий выбор запасных частей.
														Специалисты всегда готовы проконсультировать вас по вопросам подбора и покупки запчастей
														для сельскохозяйственной техники.</div>
													<div class="what-do__item-arrow">
														<svg class='about-what-do-arrow-keeper about-what-do-arrow-absolute'>
															<use class='about-what-do-arrow' xlink:href='<?=SITE_TEMPLATE_PATH;?>/assets/img/icons/icons.svg#svg-arrow-right'></use>
														</svg>
													</div>
												</div>
											</a>
										</li>

									</ul>
								</div>
							</section>
							<section class="banks about-partner">
								<div class="container">
									<h2 class="banks__title">Партнёры</h2>
									<div class="banks__inner">

										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/bryanskselmash.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/belarus-works.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/gomselmash.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/kazanselmash.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/volgograd-zavod.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/bolshaya-zemlya.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/vmrz.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/bryanskiy-zavod.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/zlin.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/belinskselmash.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/metal-fach.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/penzmash.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/ymz.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/cansa.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/tech-servic.png' alt=''></div>
										<div class='banks__img'> <img src='<?=SITE_TEMPLATE_PATH;?>/assets/img/partner/radogost.png' alt=''></div>
									</div>
								</div>
							</section>
							<section class="news">
								<div class="container">
									<div class="news__title">Больше о нас в новостях</div>
									<div class="news__inner">
										<ul class="news__list">
											<li class="news__item">
												<a class='news__item-link' href='/news-detail.php'>

													<img class="news__item-photo" src='<?=SITE_TEMPLATE_PATH;?>/assets/img/news/news1.jpg' alt=''>
													<h3 class="news__item-title">Выставка «AGRO KAVKAZ–2023»</h3>
													<div class="news__item-text">Торговый Дом «Палессе-Юг» приглашает вас посетить выставку «AGRO
														KAVKAZ–2023», которая будет проходить 1–3 февраля 2023 г. в МВЦ «МинВодыЭкспо».</div>
													<svg class='about-what-do-arrow-keeper svg-news'>
														<use class='about-what-do-arrow' xlink:href='<?=SITE_TEMPLATE_PATH;?>/assets/img/icons/icons.svg#svg-arrow-right'></use>
													</svg>
												</a>
											</li>
											<li class="news__item">
												<a class='news__item-link' href='/news-detail.php'>
													<img class="news__item-photo" src='<?=SITE_TEMPLATE_PATH;?>/assets/img/news/news2.jpg' alt=''>
													<h3 class="news__item-title">Зимняя конференция прямого посева 2023</h3>
													<div class="news__item-text">Торговый Дом «Палессе-Юг» приглашает вас посетить экспозицию нашей компании
														26–27 января 2023 г.</div>
													<svg class='about-what-do-arrow-keeper svg-news'>
														<use class='about-what-do-arrow' xlink:href='<?=SITE_TEMPLATE_PATH;?>/assets/img/icons/icons.svg#svg-arrow-right'></use>
													</svg>
												</a>
											</li>
											<li class="news__item">
												<a class='news__item-link' href='/news-detail.php'>
													<img class="news__item-photo" src='<?=SITE_TEMPLATE_PATH;?>/assets/img/news/news3.jpg' alt=''>
													<h3 class="news__item-title">ЮГАГРО — 2022</h3>
													<div class="news__item-text">Торговый Дом «Палессе-Юг» приглашает вас посетить экспозицию нашей компании
														на агропромышленной выставке «ЮГАГРО-2022»</div>
													<svg class='about-what-do-arrow-keeper svg-news'>
														<use class='about-what-do-arrow' xlink:href='<?=SITE_TEMPLATE_PATH;?>/assets/img/icons/icons.svg#svg-arrow-right'></use>
													</svg>
												</a>
											</li>
										</ul>
									</div>
								</div>
							</section>
						</div>
						<section class="feedback">
							<div class="feedback-container container">
								<div class="feedback-container_target"></div>
							</div>
							<div class="container-fluid">
								<div class="feedback__bg" style='background-image: url(<?=SITE_TEMPLATE_PATH;?>/assets/img/pages-bg-pictures/bot-form.jpg);'>
									<div class="container">
										<div class="feedback__inner">
											<div class="feedback__content">
												<form class="feedback__form" id='feedback-form'>
													<h4 class="feedback__form-title form-item">
														Остались вопросы?

														<p class='feedback__accent-color form-item'>Мы свяжемся с Вами!</p>
													</h4>
													<label for='feedback-name ' class='feedback-label form-item'>
														<input class="feedback__input feedback__name input-field-index" maxlength='40' id='feedback-name' type="text" name='Name'>
														<span class='label-span'>Имя</span>
													</label>

													<label for='feedback-phone ' class='feedback-label form-item'>
														<input class="feedback__input feedback__phone input-field-index" id='feedback-phone' type="tel" name='Phone' maxlength='20'>
														<span class='label-span'> Телефон</span>
													</label>
													<label for='feedback-topic' class='feedback-label form-item'>
														<input class="feedback__input feedback__name input-field-index" maxlength='80' id='feedback-topic' type="text" name='Topic'>
														<span class='label-span'>Тема сообщения</span>
													</label>
													<button class="red-button-link feedback__btn form-item" type="submit">Отправить заявку</button>
													<p class='feedback__form-privacy form-item'>
														Отправляя заявку Вы принимаете условия
														<a class='feedback__form-privacy-link form-item' href='/privacy.php'>Политики конфиденциальности</a>
													</p>
												</form>
											</div>
										</div>

									</div>
								</div>
							</div>
						</section>
					</section>


<?php
require($_SERVER['DOCUMENT_ROOT'].'/bitrix/footer.php');
?>